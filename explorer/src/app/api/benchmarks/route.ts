...
export async function GET() {
  try {
    ...
    if (!fs.existsSync(vizPath)) {
      console.warn(
        "[Benchmarks API] Viz path does not exist, returning empty runs"
      );
    ...
    const runs = fs
      ...
      .map((timestamp) => {
        ...
        const models = modelDirs.filter((file) => {
    ...
    console.log(
      "[Benchmarks API] Returning runs:",
      runs.map((r) => ({ timestamp: r.timestamp, modelCount: r.models.length }))
    );
    return NextResponse.json({ runs });
  } catch (error) {
    console.error("[Benchmarks API] Error reading benchmarks:", error);
    console.error(
      "[Benchmarks API] Error stack:",
