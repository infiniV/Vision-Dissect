...
export async function GET(
  ...
  try {
    ...
    console.log(`[Benchmark Metrics API] Request for timestamp: ${params.timestamp}`);
    ...
    console.log("[Benchmark Metrics API] Results file exists:", fs.existsSync(resultsPath));

    if (!fs.existsSync(resultsPath)) {
      console.warn(`[Benchmark Metrics API] Results file not found: ${resultsPath}`);
      ...
      if (fs.existsSync(resultsDir)) {
        ...
        console.log("[Benchmark Metrics API] Available files in results directory:", availableFiles);
      } else {
        console.warn("[Benchmark Metrics API] Results directory does not exist:", resultsDir);
      }
    ...
    console.log(`[Benchmark Metrics API] File size: ${fileContent.length} bytes`);
    ...
    if (Array.isArray(data.results)) {
      console.log("[Benchmark Metrics API] Using new format with 'results' array");
      resultsArray = data.results;
    } else {
      console.log("[Benchmark Metrics API] Using old format with direct model entries");
    ...
    console.log(`[Benchmark Metrics API] Found ${resultsArray.length} model results`);
    ...
    const metrics = resultsArray.map((result: any) => {
      const metric = {
        ...
        avgInference: result.avg_inference_sec || result.avg_inference_time || 0,
      ...
      console.log(`[Benchmark Metrics API] Processed metric for ${metric.model}:`, metric);
      return metric;
    });
    ...
    console.error(`[Benchmark Metrics API] Error reading benchmark metrics for timestamp ${params.timestamp}:`, error);
    console.error("[Benchmark Metrics API] Error stack:", error instanceof Error ? error.stack : 'N/A');
    return NextResponse.json({ metrics: [] });
  }
}
