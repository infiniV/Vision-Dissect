...
export async function GET(
  ...
  try {
    ...
    if (fs.existsSync(metadataPath)) {
      ...
      console.log(`[Layers API] Metadata file size: ${metadataContent.length} bytes`);
    ...
    console.log(`[Layers API] Visualization paths:`, visualizations.slice(0, 3), visualizations.length > 3 ? `... and ${visualizations.length - 3} more` : '');
    ...
    console.log(`[Layers API] Returning response with ${layers.length} layers and ${visualizations.length} visualizations`);
    return NextResponse.json(response);
  } catch (error) {
    console.error(`[Layers API] Error reading layer data for model ${params.model}:`, error);
    console.error("[Layers API] Error stack:", error instanceof Error ? error.stack : 'N/A');
    return NextResponse.json({ layers: [], visualizations: [] });
  }
}
