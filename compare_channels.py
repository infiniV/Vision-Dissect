        ...
        with torch.no_grad():
            ...
            prediction = (
                torch.nn.functional.interpolate(
                    ...
                    size=img_rgb.shape[:2] if isinstance(img_rgb, np.ndarray) else (img_rgb.height, img_rgb.width),
                    mode="bicubic",
                    align_corners=False,
                )
                .squeeze()
...
def compare_channels(image_path="test_image.jpg", sam_model_path="mobile_sam.pt"):
    ensure_viz_dir()
    ...
    if canny is not None:
        visualize_channel_grid([canny], ["Canny Edges"], "viz/canny.png", cmap="gray")
    else:
